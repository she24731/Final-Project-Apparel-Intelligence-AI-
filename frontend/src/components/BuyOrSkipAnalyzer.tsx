    ...
    <Card title="Analyze a purchase" subtitle="Upload the item you’re considering, or describe it. We’ll score fit vs your wardrobe.">
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-black/50">Candidate item (optional photo)</p>
          <div
            className={[
          ...
          >
            <div className="flex items-center gap-3">
              <div className="h-14 w-14 overflow-hidden rounded-2xl border border-line bg-[#F5F5F5]">
                {candidatePreview ? (
                  <img
              ...
              <div className="flex flex-1 items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-black">{candidate ? "Photo added" : "Drop an image here"}</p>
                  <p className="mt-1 text-xs text-black/55">Or click to choose. JPG/PNG/WebP/HEIC. Max 10MB.</p>
                </div>
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => inputRef.current?.click()}
                  className="rounded-2xl border border-line bg-[#E8E8E8] px-4 py-2 text-sm font-semibold text-black hover:border-accent/50 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  Choose photo
                </button>
                {candidate ? (
                  <button
                    ...
                    className="rounded-2xl border border-line bg-[#E8E8E8] px-4 py-2 text-sm font-semibold text-black/70 hover:border-red-400/40 hover:text-black disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    Remove
                  </button>
                ) : null}
          ...
          <label className="space-y-2 text-xs font-medium text-black/70">
            ...
            <input
              ...
              className="w-full rounded-xl border border-line bg-[#F5F5F5] px-3 py-2 text-sm text-black outline-none focus:ring-2 focus:ring-accent/30"
            />
          </label>
        </div>

        <div className="space-y-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-black/50">Candidate details</p>
        <label className="space-y-2 text-xs font-medium text-black/70">
          Category
          <select
            value={category}
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        <div>
          <p className="text-xs text-black/50">Wardrobe size: {wardrobeSize}</p>
          {wardrobeSize === 0 ? (
        ...
        <button
          ...
          className="rounded-xl border border-line bg-[#C8A96A] px-4 py-2 text-sm font-semibold text-black hover:border-accent/50 disabled:cursor-not-allowed disabled:opacity-40"
        >
      ...
      {!result ? (
        <div className="mt-5 rounded-xl border border-dashed border-line/80 bg-[#E8E8E8]/30 p-6 text-center text-sm text-black/55">
        ...
        <div className="mt-5 space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="rounded-xl border border-line bg-[#E8E8E8] p-4">
              <p className="text-xs font-semibold uppercase tracking-wide text-black/50">Verdict</p>
              <p className="mt-2 text-2xl font-semibold text-black">{result.recommendation}</p>
            </div>
          </div>

          {result.decision_criteria?.length ? (
            <div className="rounded-2xl border border-line bg-[#E8E8E8]/30 p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-black/55">Verdict criteria</p>
              <ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-black/60">
          ...
          {result.outfit_suggestions?.length ? (
            <div className="rounded-2xl border border-line bg-[#E8E8E8]/30 p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-black/55">Fits with these outfits</p>
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-3">
                {result.outfit_suggestions.map((sug, idx) => (
                  <div className="rounded-2xl border border-line bg-[#E8E8E8] p-4">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-black/55">
                      Combo {idx + 1}{sug.occasion ? ` · ${sug.occasion}` : ""}
                    </p>
                    <p className="mt-2 text-sm font-semibold text-black">{sug.title}</p>
                    {sug.description ? <p className="mt-1 text-xs text-black/55">{sug.description}</p> : null}
                    <div className="mt-3 flex flex-wrap gap-2">
                      {sug.garment_ids.map((gid) => {
                        ...
                        return (
                          <div className="flex items-center gap-2 rounded-xl border border-line bg-[#E8E8E8] px-2 py-2">
                            <div className="h-9 w-9 overflow-hidden rounded-lg border border-line bg-[#F5F5F5]">
                              <img
                                src={src}
                            ...
                            <div>
                              <p className="text-xs font-semibold text-black">{g.category}</p>
                              <p className="text-[11px] text-black/55">{g.color}</p>
                            </div>
                          </div>
                        );
                      })}
          ...
          <div>
            <p className="text-sm text-black/80">{result.explanation}</p>
            <ul className="mt-3 list-disc space-y-1 pl-5 text-xs text-black/60">
...
function ScoreTile({ label, value, isInt }: { label: string; value: number; isInt?: boolean }) {
  return (
    <div className="rounded-xl border border-line bg-[#E8E8E8] p-4">
      ...
      <p className="mt-2 text-3xl font-semibold text-black">{isInt ? value : value.toFixed(2)}</p>
    </div>
  );
}
